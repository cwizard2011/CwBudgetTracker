...
export const BudgetProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addBudget: BudgetContextValue['addBudget'] = async ({ title, amountPlanned, period, category, categoryIcon, recurring = 'none', anchorDateISO, dateISO, recurringStopISO, notes, items, currency }) => {
    const now = Date.now();
    ...
    const baseBudget: Budget = {
      ...
      createdAt: now,
      updatedAt: now,
      items: items.map(item => ({ ...item, id: generateUniqueId(item.id, dateISO || 'unknown'), parent_id: baseId })),
    };
